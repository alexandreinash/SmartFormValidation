...
const apiLimiter = rateLimit({
  ...
  max: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || (isProduction ? 100 : 1000), // 1000 in dev, 100 in production
  message: {
    success: false,
...
const submissionMax = process.env.RATE_LIMIT_SUBMISSION_MAX 
  ...
  : (isProduction ? 10 : 1000); // 1000 in dev, 10 in production
...
const authMax = process.env.RATE_LIMIT_AUTH_MAX 
  ...
  : (isProduction ? 5 : 50); // 50 in dev, 5 in production
